...
The results for the skin lesion classification indicate that models that were fine-tuned with close-up images achieved an unremarkable performance. When compared with the models from the first scenario, fine-tuned with dermatoscopy images, the accuracy was considerably lower.

The models from the first scenario achieved a better accuracy with much smaller training volumes, comparatively, the model fine-tuned with \gls{qlora} and 5000 dermatoscopy images had an accuracy that surpassed the one from the model trained with \gls{qlora} and 4,736 close-up images in 42,2\%. \autoref{tab:overall_comparison_skin_lesions} shows the accuracy for each fine-tuned model.
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption{Accuracy in the classification of skin lesions for the second scenario. The highest values are highlighted in bold.}
    \label{tab:overall_comparison_risk}
\end{table}

\subsection{Performance in Report Generation}

The performance in report generation was evaluated with three \gls{nlp} metrics: \gls{bleu}, \gls{rouge}, and \gls{meteor}. The results indicate that the fine-tuned perform much better than the base model in this task. Both, the models trained with \gls{qlora} and \gls{lora} managed to generalize the report format, presenting the recommendations in a similar way to a real dermatologist. The greatest improvement was observed with \gls{bleu} and \gls{meteor} metrics. \autoref{fig:nlp_metrics} depics the \gls{nlp} metrics for the models.

The reports generated by \gls{llama}-3.2-11B followed the specified template and used a medical language, but they failed to provide the expected recommendations. Besides, the reports generated by the fine-tuned models also presented some problems, such as the presence of gramatical errors, which could be explained by errors in the source dataset itself.

\subsection{Comparison between QLoRA and LoRA}

Lastly, when comparing the training metrics presented in the \Cref{fig:duration,fig:memory}, and the \autoref{tab:final_training_metrics}, we noticed that both fine-tuning methods require similar amounts of time to be concluded. Furthermore, \gls{lora} tends to alocate more memory. In the first scenario, \gls{lora} had an average memory utilization 84.19\% greater than \gls{qlora}. While in the second scenario, \gls{lora} used 25.94\% more memory. The discrepancy in the difference between the memory utilization of both methods for the first and second scenario can be explained by the change in the hyperparameters that affect the memory usage, such as the device batch size and the gradient accumulation step. Moreover, the aspects of each dataset can also influence the memory utilization.
